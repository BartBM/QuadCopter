...
void InvMpu::init(I2CDataBus *i2cDataBus)
{
    I2CChip::init(i2cDataBus);

}

int InvMpu::i2c_write(unsigned char write_addr, unsigned char reg_addr, int numberOfBytes, unsigned char* data)
{
    vector<unsigned char> dataToSend;
    dataToSend.push_back(reg_addr);
...
int InvMpu::i2c_read(unsigned char write_addr, unsigned char reg_addr, int numberOfBytes, unsigned char* data)
{
    vector<unsigned char> dataToRead = readValues<unsigned char>(reg_addr, numberOfBytes);
    for (int i=0; i<numberOfBytes; i++) {
        data[i] = dataToRead[i];
    }
    return 0;
 ...
 */
int InvMpu::set_int_enable(unsigned char enable)
{
    unsigned char tmp;
 ...
 *  @return     0 if successful.
 */
int InvMpu::mpu_init(struct int_param_s *int_param)
{
 ...
 *  @return     0 if successful.
 */
int InvMpu::mpu_lp_accel_mode(unsigned short rate)
{
    unsigned char tmp[2];
...
int InvMpu::mpu_read_6500_gyro_bias(long *gyro_bias) {
	unsigned char data[6];
 ...
 *  @return     0 if successful.
 */
int InvMpu::mpu_get_compass_sample_rate(unsigned short *rate)
{
#ifdef AK89xx_SECONDARY
 ...
 *  @return     0 if successful.
 */
int InvMpu::mpu_set_compass_sample_rate(unsigned short rate)
{
#ifdef AK89xx_SECONDARY
    unsigned char div;
...
int InvMpu::get_st_biases(long *gyro, long *accel, unsigned char hw_test)
{
    unsigned char data[MAX_PACKET_LENGTH];
...
int InvMpu::accel_6500_self_test(long *bias_regular, long *bias_st, int debug)
{
    ...
    if (i2c_read(st.hw->addr, REG_6500_XA_ST_DATA, 3, regs)) {
    	if(debug)
            printf("Reading OTP Register Error.\n");
    	return 0x07;
    }
    if(debug)
        printf("Accel OTP:%d, %d, %d\n", regs[0], regs[1], regs[2]);
	for (i = 0; i < 3; i++) {
		if (regs[i] != 0) {
	...
	/* Self Test Pass/Fail Criteria C */
		...
		if(debug)
            printf("Accel:CRITERIA C: bias less than %7.4f\n", accel_offset_max/1.f);
		for (i = 0; i < 3; i++) {
			if(fabs(bias_regular[i]) > accel_offset_max) {
				if(debug)
                    printf("FAILED: Accel axis:%d = %ld > 500mg\n", i, bias_regular[i]);
				result |= 1 << i;	//Error condition
			}
		}
	}
...
int InvMpu::gyro_6500_self_test(long *bias_regular, long *bias_st, int debug)
{
    int i, result = 0, otp_value_zero = 0;
	...
	if(otp_value_zero == 0) {
		...
		for (i = 0; i < 3; i++) {
			...
			if(st_shift_cust[i] < gyro_st_al_max) {
				if(debug)
                    printf("GYRO FAIL axis:%d greater than 60dps\n", i);
				result |= 1 << i;	//Error condition
			}
		}
	}
	...
	/* Self Test Pass/Fail Criteria C */
		...
		if(debug)
            printf("Gyro:CRITERIA C: bias less than %7.4f\n", gyro_offset_max/1.f);
		for (i = 0; i < 3; i++) {
			if(fabs(bias_regular[i]) > gyro_offset_max) {
				if(debug)
                    printf("FAILED: Gyro axis:%d = %ld > 20dps\n", i, bias_regular[i]);
				result |= 1 << i;	//Error condition
			}
		}
	}
...
int InvMpu::get_st_6500_biases(long *gyro, long *accel, unsigned char hw_test, int debug)
{
    ...
    if(debug)
        printf("Starting Bias Loop Reads\n");
 ...
 */
int InvMpu::mpu_run_6500_self_test(long *gyro, long *accel, unsigned char debug)
{
    ...
    if (ii == tries) {
         ...
         */
        if(debug)
            printf("Retrieving Biases Error - possible I2C error\n");
    ...
    if (ii == tries) {
        ...
        if(debug)
            printf("Retrieving ST Biases Error - possible I2C error\n");
    ...
    if(debug)
        printf("Accel Self Test Results: %d\n", accel_result);

    gyro_result = gyro_6500_self_test(gyro, gyro_st, debug);
    if(debug)
        printf("Gyro Self Test Results: %d\n", gyro_result);
    ...
    if(debug)
        printf("Compass Self Test Results: %d\n", compass_result);
    if (!compass_result)
        result |= 0x04;
#else
    result |= 0x04;
 ...
 *  @return     0 if successful.
 */
int InvMpu::mpu_get_compass_reg(short *data, unsigned long *timestamp)
{
#ifdef AK89xx_SECONDARY
    unsigned char tmp[9];
 ...
 *  @return     0 if successful.
 */
int InvMpu::mpu_get_compass_fsr(unsigned short *fsr)
{
#ifdef AK89xx_SECONDARY
