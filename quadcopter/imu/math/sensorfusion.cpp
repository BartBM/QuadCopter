...
void SensorFusion::processEvent(Event* event)
{
    if (ImuEvent* imuEvent = dynamic_cast<ImuEvent*>(event)) {
        madgwickAHRSupdateIMU(imuEvent->getGyroscopeReading().getX() * PI / 180.0f,
                              imuEvent->getGyroscopeReading().getY() * PI / 180.0f,
                              imuEvent->getGyroscopeReading().getZ() * PI / 180.0f,
                              imuEvent->getAccelerometerReading().getX(),
                              imuEvent->getAccelerometerReading().getY(),
                              imuEvent->getAccelerometerReading().getZ());
